...
var _lodash9 = require('lodash.mergewith');

var _lodash10 = _interopRequireDefault(_lodash9);

var _lodash11 = require('lodash.intersection');
...
function firebaseDenormalizer(firebase) {
  ...
  return function modelDenormalizer(modelName) {
    ...
    async function nonFilterableProperty(propName) {
      filterables = (0, _lodash8.default)(filterables, propName);
    ...
    function getFiltersIntersection(filterResults) {
      const mergedFilterResults = filterResults.reduce((acc, val) => (0, _lodash10.default)(acc, val, fullObjectPriority), {});
      const ids = filterResults.map(filterResult => Object.keys(filterResult));
      const intersectionIds = _lodash12.default.apply(null, ids);
      return (0, _lodash14.default)(mergedFilterResults, intersectionIds);
    }
...
function getPathString(...paths) {
  paths = (0, _lodash6.default)((0, _lodash2.default)(paths));
  return `${(0, _lodash6.default)((0, _lodash2.default)(paths.map(path => path.split('/')))).join('/')}`.replace(/\/$/, '');
}